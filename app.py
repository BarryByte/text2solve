...
# --- Firestore Functions ---
def save_qa_to_firestore(question: str, solution: str):
    if not (firebase_initialized and db and question and solution): return False
    try:
        db.collection('q_and_a').add({
            'question': question,
            'solution': solution,
...
def get_past_qa():
    if not (firebase_initialized and db): return []
    docs = db.collection('q_and_a').order_by('timestamp', direction=firestore.Query.DESCENDING).stream()
    past = []
    for d in docs:
        data = d.to_dict()
...
def generate_hint(question: str):
    prompt = f"""
You are an expert tutor. Provide 2-3 concise hints (no full solution) to help the student approach the problem.
Question: "{question}"
Hints:
"""
    try:
...
def generate_solution(question: str):
    prompt = f"""
You are an expert tutor. Provide a clear, step-by-step solution explaining each formula and step.
Question: "{question}"
Solution:
"""
    try:
...
# --- Session State ---
for key in ['current_question','current_hint','current_solution','hint_generated','solution_generated']:
    if key not in st.session_state:
        st.session_state[key] = '' if 'current' in key else False
...
past = get_past_qa()
if past:
    for qa in past[:5]:  # show recent 5
        with st.sidebar.expander(f"Q: {qa['question']}", expanded=False):
...
# --- Main UI ---
st.title("📚 Text-to-Solution Generator")
st.markdown("Enter your math or physics question below.")

st.subheader("❓ Ask a New Question")
user_q = st.text_area("Your Question:", value=st.session_state.current_question, height=100)
...
with col1:
    if st.button("💡 Get Hint"):
        if not user_q.strip():
            ...
            st.session_state.current_hint = generate_hint(user_q)
            st.session_state.hint_generated = True
with col2:
    if st.session_state.hint_generated and st.button("✨ Show Solution"):
        st.session_state.current_solution = generate_solution(st.session_state.current_question)
        st.session_state.solution_generated = True
        # Save
        if st.session_state.current_solution and not st.session_state.current_solution.startswith('Error'):
            if save_qa_to_firestore(st.session_state.current_question, st.session_state.current_solution):
                st.success("Saved to history!")
                st.cache_data.clear()

# --- Display Hint and Solution ---
if st.session_state.hint_generated:
    st.subheader("📝 Hints")
...
# --- Footer ---
st.markdown("---")
st.markdown("Built with Gemini & Firebase 🔥")
